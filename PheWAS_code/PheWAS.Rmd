...
prs_merged_AFR = pheno_merged %>% filter(assigned_anc == "AFR")
prs_merged_AMR = pheno_merged %>% filter(assigned_anc == "AMR")
prs_merged_EAS = pheno_merged %>% filter(assigned_anc == "EAS")
prs_merged_SAS = pheno_merged %>% filter(assigned_anc == "SAS")
# Function needed
...
demo_df = prs_atlas_final %>% mutate(UniqueSampleId = as.integer(IID)) %>% 
  dplyr::select(UniqueSampleId, prs_eurnorm, assigned_anc) %>% distinct() %>% 
  inner_join(demographic, by = c("UniqueSampleId" = "UniqueSampleId")) %>% 
  mutate(Age = round(as.numeric((as.Date(phe_table_extract_date)-as.Date(BirthDate))/365.25))) %>% 
  dplyr::select(UniqueSampleId, Age, Sex, prs_eurnorm, assigned_anc) %>% distinct() %>% filter(Sex != "X") %>% 
  inner_join(safejoin_icd, by = c("UniqueSampleId" = "UniqueSampleId")) %>% distinct() %>% 
  mutate(AD = case_when(
    `290.11` == 0 ~ 0,
    `290.11` >= 1 ~ 1
  ...
  dplyr::select(UniqueSampleId, Age, Sex, assigned_anc, AD_cat, prs_eurnorm) 
# Subset the phecode table to samples in the table above, calculate phecode-related characteristics
clinical_df = demo_df %>% dplyr::select(UniqueSampleId, assigned_anc) %>% left_join(safejoin_icd) %>% 
  mutate(unique_diagnosis = dplyr::select(., `008`:`915`) %>% mutate_all(scale1) %>% rowSums(na.rm = T)) %>% 
  dplyr::select(UniqueSampleId, assigned_anc, unique_diagnosis)
clinical_df %>% dplyr::group_by(assigned_anc) %>% summarise(med_visit = median(unique_diagnosis))
# Then calculate the mean/median for encounter facts
# Read in ATLAS files
enc_raw = read.csv(file = "~/Downloads/BioBank_enc_0721.csv", header = T)
encounter_df = demo_df %>% dplyr::select(UniqueSampleId, assigned_anc) %>% 
  left_join(enc_raw, by = c("UniqueSampleId" = "UniqueSampleId")) %>% 
  mutate(StartDate = as.Date(StartDate)) %>% 
  dplyr::select(UniqueSampleId, assigned_anc, DiagnosisCode, StartDate) %>% 
...
table_uni = descrTable(assigned_anc ~ Sex + AD_cat + prs_eurnorm + Age + n_encounter + n_diagnosis + time_span, 
...
pheno_df_pre = prs_merged_EUR %>% select(-c(UniqueSampleId, prs_eurnorm, assigned_anc)) %>% 
...
pheno_df_pre = prs_merged_EUR_noAD %>% select(-c(UniqueSampleId, prs_eurnorm, assigned_anc)) %>% 
  select_if(function(col) sum(col, na.rm = T) >= 50) %>% mutate_all(scale1)
...
# Run PheWAS
result_firth_EUR = phewas_ext(names(pheno_df)[-1], predictors = names(prs_df)[-1], cores = 4,
...
result_sig_EUR = result_sig_EUR_filter %>%
...
# AFR
start_time = Sys.time()
pheno_df_pre = prs_merged_AFR %>% select(-c(UniqueSampleId, prs_eurnorm, assigned_anc)) %>% 
  select_if(function(col) sum(col, na.rm = T) >= 10) %>% mutate_all(scale1)
...
end_time = Sys.time()
end_time - start_time
result_sig_AFR_filter = result_firth_AFR %>% filter(n_cases >= 10) %>% addPhecodeInfo()
...
# AMR
start_time = Sys.time()
pheno_df_pre = prs_merged_AMR %>% select(-c(UniqueSampleId, prs_eurnorm, assigned_anc)) %>% 
  select_if(function(col) sum(col, na.rm = T) >= 10) %>% mutate_all(scale1)
...
end_time = Sys.time()
end_time - start_time
result_sig_AMR_filter = result_firth_AMR %>% filter(n_cases >= 10) %>% addPhecodeInfo()
...
# EAS
start_time = Sys.time()
pheno_df_pre = prs_merged_EAS %>% select(-c(UniqueSampleId, prs_eurnorm, assigned_anc)) %>% 
  select_if(function(col) sum(col, na.rm = T) >= 10) %>% mutate_all(scale1)
...
end_time = Sys.time()
end_time - start_time
result_sig_EAS_filter = result_firth_EAS %>% filter(n_cases >= 10) %>% addPhecodeInfo()
...
# SAS
start_time = Sys.time()
pheno_df_pre = prs_merged_SAS %>% select(-c(UniqueSampleId, prs_eurnorm, assigned_anc)) %>% 
  select_if(function(col) sum(col, na.rm = T) >= 10) %>% mutate_all(scale1)
...
end_time = Sys.time()
end_time - start_time
result_sig_SAS_filter = result_firth_SAS %>% filter(n_cases >= 10) %>% addPhecodeInfo()
...
```{r}
# Write outputs
sheets_PheWAS = list('EUR' = result_sig_EUR, 'AFR' = result_sig_AFR, 'AMR' = result_sig_AMR,
                     'EAS' = result_sig_EAS, 'SAS' = result_sig_SAS)
write_xlsx(sheets_PheWAS, path = paste0(output_path, 'PheWAS_ancestry.xlsx'))
```

### Plot EUR results (Manhattan plot)
```{r}
result_sig_EUR_sort = result_sig_EUR %>% arrange(phenotype)
pdf(paste0(output_path, 'PheWAS_EUR.pdf'), width = 20, height = 10)
ggplot(result_sig_EUR_sort, aes(x = phenotype, y = -log(p_adj_fdr))) + 
  ...
  geom_text_repel(data = . %>% mutate(label = ifelse(p_adj_fdr <= 0.15, as.character(description), "")), 
                  aes(label = label), size = 5, box.padding = unit(0.7, "lines")) + 
  geom_hline(yintercept = -log(0.05), color = "red", size = 1, alpha = 0.5) 
dev.off()
```
